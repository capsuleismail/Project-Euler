res = [[8, 2, 22, 97, 38, 15, 00, 40, 00, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8],
...
for i in range(0, 20):
    for j in range(0, 20):
        ...
        if j < 17:
            first = res[i][j] * res[i][j + 1] * res[i][j + 2] * res[i][j + 3]
        if i < 17:
            second = res[i][j] * res[i + 1][j] * res[i + 2][j] * res[i + 3][j]
        if j < 17 and i > 2:
            third = res[i][j] * res[i - 1][j + 1] * \
                res[i - 2][j + 2] * res[i - 3][j + 3]
        if j < 17 and i < 17:
            forth = res[i][j] * res[i + 1][j + 1] * \
                res[i + 2][j + 2] * res[i + 3][j + 3]
        deep = max([first, second, third, forth])
        if deep > result:
            result = deep
print(result)
